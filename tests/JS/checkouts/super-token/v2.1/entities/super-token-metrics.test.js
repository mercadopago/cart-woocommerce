...
describe('MPSuperTokenMetrics.waitForMelidata_() and timeout race (PSW-4050)', () => {
  ...
  describe('sendMetric()', () => {
    test('Given an errorCode, When sendMetric() is called, Then should execute without errors', () => {
      expect(() => {
        metrics.sendMetric('test_metric', 'value', 'message', 'some_error_code');
      }).not.toThrow();
    });

    test('Given no errorCode, When sendMetric() is called, Then should execute without errors', () => {
      expect(() => {
        metrics.sendMetric('test_metric', 'value', 'message');
      }).not.toThrow();
    });
