...
- **Full matrix / release:** `bash e2e/run-all-report.sh` (7 countries × 2 modes, consolidated report). It runs each site in two phases so store-mutating specs stay isolated — see the `@serial-store` trap in traps.md.
- **Subscription specs** (`tests/**/subscriptions/`, `tests/subscriptions/`) require the paid **WooCommerce Subscriptions** plugin: set `WCS_ZIP_PATH` in `e2e/.env` to the zip's path **inside the container** — put it under `e2e/plugins/` (gitignored; mounted at `/woocommerce-mercadopago/e2e/plugins/`) so it survives store resets. Without it, those specs skip/fail on WCS install.
- **Local retries** are 1 (`playwright.config.js`); full-checkout flows against the shared MP sandbox can flake — run store-mutating/full-checkout specs with `--workers=1` (or via `run-all-report.sh`, which isolates them).
