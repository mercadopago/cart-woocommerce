...
| `src/SuperToken/` | Reusable, gateway-agnostic Super Token payment orchestration. `SuperTokenPaymentProcessor` runs validate → transaction → metadata → response; `SuperTokenValidator` handles required fields + Datadog telemetry; `SuperTokenCheckout` is the checkout value object. `Contracts/` + `Adapters/` are ports (transaction factory, metadata writer) so other methods (e.g. a future Pix flow) can reuse the flow. `CustomGateway` delegates the `super_token` checkout here. |
...
| `assets/js/checkouts/super-token/` | Super Token CVV tokenization runtime — built locally and loaded from the Mercado Pago CDN in production. The hexagonal **ES module + TypeScript** tree (`ports/`, `core/`, `useCases/`, `adapters/`, `composition/`) is the runtime in every mode. In dev/self-construct, the unified `bootstrap.ts` entry builds and publishes the instances and serves one CSS containing both `data-variant` scopes. In production 8.9.3, the retrocompatible A/B loader remains active and chooses one refactored, variant-frozen bundle and stylesheet per path (`v1/` → v2, `v2.1/` → v2.1). The unified `mp-super-token/` JS/CSS pair is staged only for the deferred TASK-013 path cutover. Both variants share the typed-row builder and the Classic/Blocks finalization use case. This is the only TypeScript in the repo. |
...
| `packages/narciso/components/input-document/` | Holder-document field. `DocumentHandlerFactory` + per-type handlers own the per-site mask (`mask()`) and permissive interim validation (`validate()`); `InputDocument.setInputProperties` derives the DOM `maxlength` from the mask so it never gates below develop. See traps.md. |
...
4. `proccessPaymentInternal()` dispatches on `checkout_type`: `super_token` delegates to `SuperToken\SuperTokenPaymentProcessor::process()` (gateway-agnostic orchestration), while `wallet_button` and the default `custom` path are handled inline. Each path instantiates the matching `{Method}Transaction`, which builds the Mercado Pago API payload.
...
**Refund reconciliation & deduplication (PSW-4308):**
A refund reaches an order from two origins — the WooCommerce admin panel (`RefundHandler::processRefund` → MP refund endpoint) and an async MP refund notification (routed to `OrderStatus::refundedFlow` by `WebhookNotification` / `CoreNotification`). Two independent barriers keep the same refund from being applied twice:
1. **Value barrier** (`OrderStatus::refundAlreadyProcessed`) — skips the notification when `totalRefundedMP <= totalRefundedWC`. Runs first, and is the **only** barrier available to Super Token.
2. **Refund-id barrier** (`OrderStatus::isRefundIdApplied` + `OrderMetadata::addAppliedRefundId`) — persists every applied `refund_id` in the `_mp_applied_refund_ids` order meta (JSON array, reload-before-write via `read_meta_data(true)`, HPOS-safe). `RefundHandler` writes it at refund time when the response carries an id; `refundedFlow` writes it after `wc_create_refund` succeeds. This catches panel-originated refunds on non-Super-Token methods.

Super Token's refund endpoint returns HTTP 200 with an **empty body** (no `refund_id`), so barrier 2 is unavailable to it — ST dedup rests entirely on barrier 1. A skipped notification still keeps the `[Refund X]` payment metadata current. See [traps.md](traps.md#refund-deduplication-psw-4308-super-token-has-no-refund_id).
