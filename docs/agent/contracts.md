...
| `external` | `POST /ppcore/prod/transaction/v1/payments/{payment_id}/refund` via `src/Helpers/Requester.php` (`RefundHandler`) | Panel-initiated refund (all payment methods, incl. Super Token). Sends header `x-idempotency-key = sha256(payment_id\|amount\|order_id\|already_refunded_before)` — deterministic anti-double-click barrier (DD-3). `already_refunded_before` excludes the current refund and is in the same currency as `amount`. | SDK default | none (single attempt — `Requester::post` does not retry) | API errors mapped by `RefundStatusCodes` and surfaced to the admin as a WooCommerce refund error | — |
