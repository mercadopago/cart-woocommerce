...
// MCO settles in COP; forcing the store to USD makes the store currency differ from the
// account currency, which is the precondition for currency_ratio conversion.
const STORE_CURRENCY = "USD";
// global-setup.js prices MCO products at 100000 (tuned for native COP nominal scale). Once the
// store currency is USD, that same 100000 gets re-scaled by the live ~3074 COP/USD ratio into an
// unrealistic ~307M COP charge, which MP's API rejects ("Invalid installments"). Reprice to a
...
test.describe("currency conversion @serial-store", () => {
  test("Given the store currency (USD) differs from the MCO account currency (COP) and conversion is on, When the buyer pays with an approved card, Should charge using the live ppcore exchange ratio", async ({ page }) => {
    ...
    await goToCustomCheckout(page, shop_url, guestUserMCO);
    await captureCurrencyScreenshot(page, "mco-01-checkout-before-payment-usd.png");
    ...
    await captureCurrencyScreenshot(page, "mco-02-order-received.png");
    ...
    const { storedRatio, liveRate } = await assertCurrencyRatioMatchesLiveRate(
      orderId,
      process.env.MP_ACCESS_TOKEN_TEST_MCO,
      STORE_CURRENCY,
      "beta"
    );
    writeCurrencyProof("mco-03-ratio-proof.json", {
      site: "MCO",
      order_id: orderId,
