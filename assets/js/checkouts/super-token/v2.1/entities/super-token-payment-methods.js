...
class MPSuperTokenPaymentMethods {
    ...
    MERCADO_PAGO_CREDIT_CARD_NAME = wc_mercadopago_supertoken_bundle_params.mercado_pago_credit_card_name;
    ...
    MP_LOGO_BLUE_PATH = wc_mercadopago_supertoken_bundle_params.mp_logo_blue_path;
    MP_LOGO_DARK_PATH = wc_mercadopago_supertoken_bundle_params.mp_logo_dark_path;
    SAVED_CARDS_TITLE = wc_mercadopago_supertoken_bundle_params.saved_cards_title;
    SAVED_CARD_TITLE = wc_mercadopago_supertoken_bundle_params.saved_card_title;
    MP_METHODS_TITLE = wc_mercadopago_supertoken_bundle_params.mp_methods_title;
    ACCOUNT_MONEY_BALANCE_TEXT = wc_mercadopago_supertoken_bundle_params.account_money_balance_text;
    SAVED_PAYMENT_METHOD_TITLE = wc_mercadopago_supertoken_bundle_params.saved_payment_method_title;
    ...
    constructor(mpSdkInstance, mpSuperTokenMetrics, wcEmailListener = null) {
    ...
    isMercadoPagoCard(paymentMethod) {
        return paymentMethod?.type === this.PREPAID_CARD_TYPE && paymentMethod?.issuer?.name?.toLowerCase()?.includes(this.MERCADO_PAGO_ISSUER_NAME);
    }

    isMercadoPagoCreditCard(paymentMethod) {
        return paymentMethod?.type === this.CREDIT_CARD_TYPE && paymentMethod?.issuer?.name?.toLowerCase()?.includes(this.MERCADO_PAGO_ISSUER_NAME);
    }
    ...
    groupPaymentMethods(paymentMethods) {
        const cardTypes = [this.CREDIT_CARD_TYPE, this.DEBIT_CARD_TYPE, this.PREPAID_CARD_TYPE];
        const cardPaymentMethods = paymentMethods.filter(paymentMethod => cardTypes.includes(paymentMethod?.type)).slice(0, 3);
        const otherPaymentMethods = paymentMethods.filter(paymentMethod => !cardTypes.includes(paymentMethod?.type));
        return { cardPaymentMethods, otherPaymentMethods };
    }

    getMpCardThumbnailPath(siteId, iconPaths) {
        // MLB and unlisted sites default to the dark icon; only MLA and MLM use blue.
        const blueSites = ['MLA', 'MLM'];
        return blueSites.includes(siteId?.toUpperCase()) ? iconPaths?.blue : iconPaths?.dark;
    }

    buildBlockHeader(title, blockHeader = null) {
        const header = document.createElement('header');
        header.classList.add(this.SUPER_TOKEN_STYLES.BLOCK_HEADER);

        if (blockHeader) {
            const emailHtml = this.wcEmailListener?.isValid(blockHeader?.email)
                ? `<span class="${this.SUPER_TOKEN_STYLES.BLOCK_EMAIL}">${this.escapeHtml(blockHeader.email)}</span>`
                : '';
            header.innerHTML = `
                <div class="${this.SUPER_TOKEN_STYLES.BLOCK_HEADER_INFO}">
                    <span class="${this.SUPER_TOKEN_STYLES.BLOCK_TITLE}">${this.escapeHtml(title)}</span>
                    ${emailHtml}
                </div>
                <img alt="" aria-hidden="true" class="${this.SUPER_TOKEN_STYLES.BLOCK_HEADER_LOGO}" src="${this.escapeHtml(blockHeader?.icon ?? '')}">
            `;
        } else {
            // Intentionally no BLOCK_HEADER_INFO wrapper — setupEmailHeaderListener relies on its absence here.
            header.innerHTML = `<span class="${this.SUPER_TOKEN_STYLES.BLOCK_TITLE}">${this.escapeHtml(title)}</span>`;
        }

        return header;
    }

    renderSavedCardsBlock(cardPaymentMethods, blockHeader, title = this.SAVED_CARDS_TITLE) {
        if (!cardPaymentMethods?.length) return;

        const section = document.createElement('section');
        section.classList.add(
            this.SUPER_TOKEN_STYLES.BLOCK,
            this.SUPER_TOKEN_STYLES.BLOCK_SAVED_CARDS
        );
        section.setAttribute('role', 'group');
        section.setAttribute('aria-label', title);
        section.setAttribute('tabindex', '0');

        section.appendChild(this.buildBlockHeader(title, blockHeader));

        cardPaymentMethods.forEach(paymentMethod => section.appendChild(this.createPaymentMethodElement(paymentMethod)));

        const container = this.getCustomCheckoutEntireElement();
        container.insertBefore(section, container.firstChild);
    }

    renderOtherMpMethodsBlock(otherPaymentMethods, title = this.MP_METHODS_TITLE, blockHeader = null) {
        if (!otherPaymentMethods?.length) return;

        const section = document.createElement('section');
        section.classList.add(
            this.SUPER_TOKEN_STYLES.BLOCK,
            this.SUPER_TOKEN_STYLES.BLOCK_OTHER_MP_METHODS
        );
        section.setAttribute('role', 'group');
        section.setAttribute('aria-label', title);
        section.setAttribute('tabindex', '0');
        ...
        otherPaymentMethods.forEach(paymentMethod => section.appendChild(this.createPaymentMethodElement(paymentMethod)));
    ...
    removeAccountMoneyBalanceLine() {
        ...
        const balanceLine = customCheckoutRoot
            .querySelector(`.${this.SUPER_TOKEN_STYLES.ACCOUNT_MONEY_BALANCE_LINE}`);
    ...
    setupEmailHeaderListener() {
        if (this.emailHeaderListenerRegistered || !this.wcEmailListener) return;
        this.emailHeaderListenerRegistered = true;
        this.wcEmailListener.onEmailChange((email, isValid) => {
            try {
                const headerInfo = this.getCustomCheckoutEntireElement()?.querySelector(
                    `.${this.SUPER_TOKEN_STYLES.BLOCK_HEADER_INFO}`
                );
                if (!headerInfo) return;
                const existingSpan = headerInfo.querySelector(`.${this.SUPER_TOKEN_STYLES.BLOCK_EMAIL}`);
                if (isValid) {
                    if (existingSpan) {
                        existingSpan.textContent = email;
                    } else {
                        const emailSpan = document.createElement('span');
                        emailSpan.classList.add(this.SUPER_TOKEN_STYLES.BLOCK_EMAIL);
                        emailSpan.textContent = email;
                        headerInfo.appendChild(emailSpan);
                    }
                } else {
                    existingSpan?.remove();
                }
            } catch (e) {
                window.console?.warn?.('ST: email header update failed', e);
            }
        });
    }
    ...
    buildPaymentMethodAriaLabel(paymentMethod, lastFourDigits, installmentsWithoutFee) {
        const name = (paymentMethod?.name ?? '').replace(/&nbsp;/g, ' ');
        const lastFourDigitsText = lastFourDigits ? ` ${this.LAST_DIGITS_TEXT} ${lastFourDigits}` : '';
        const shouldIncludeInstallmentsPill = (this.isCreditCard(paymentMethod) || this.isConsumerCredits(paymentMethod)) && installmentsWithoutFee > 1;
        const installmentsWithoutFeeText = shouldIncludeInstallmentsPill
    ...
    createPaymentMethodElement(paymentMethod) {
        const lastFourDigits = this.isMercadoPagoCreditCard(paymentMethod)
            ? null
            : paymentMethod?.card?.card_number?.last_four_digits;
        ...
        if (this.isAccountMoney(paymentMethod)) {
            paymentMethodElement.classList.add(this.SUPER_TOKEN_STYLES.ACCOUNT_MONEY_ROW);
        }
        paymentMethodElement.setAttribute('aria-label', ariaLabel);
        paymentMethodElement.dataset.baseAriaLabel = ariaLabel;
        ...
        const thumbnailSrc = this.isMercadoPagoCreditCard(paymentMethod)
            ? (this.getMpCardThumbnailPath(this.SITE_ID, this.getMpIconPaths()) || paymentMethod?.thumbnail)
        ...
        paymentMethodElement.innerHTML = `
            <section class="${this.SUPER_TOKEN_STYLES.PAYMENT_METHOD_HEADER}">
                <figure class="${this.SUPER_TOKEN_STYLES.PAYMENT_METHOD_THUMBNAIL}">
                        <img src="${this.escapeHtml(thumbnailSrc ?? '')}" alt="" aria-hidden="true">
                </figure>
    ...
    normalizeAccountPaymentMethods(accountPaymentMethods) {
        return accountPaymentMethods.map(paymentMethod => {
            ...
            if (this.isCreditCard(paymentMethod) || this.isDebitCard(paymentMethod)) {
                if (this.isMercadoPagoCreditCard(paymentMethod)) {
                    paymentMethod.thumbnail = this.getMpCardThumbnailPath(this.SITE_ID, this.getMpIconPaths())
                        || paymentMethod.thumbnail;
                    paymentMethod.name = this.MERCADO_PAGO_CREDIT_CARD_NAME || paymentMethod.name;
                } else {
                    paymentMethod.thumbnail = this.PAYMENT_METHODS_THUMBNAILS[paymentMethod.id] || paymentMethod.thumbnail || this.WHITE_CARD_PATH;
    ...
    organizePaymentMethodsElements(paymentMethods) {
        ...
        const { cardPaymentMethods, otherPaymentMethods } = this.groupPaymentMethods(normalizedPaymentMethods);

        const blockHeader = {
            email: this.wcEmailListener?.getEmail() || this.CURRENT_USER_EMAIL,
            icon: this.NEW_MP_LOGO_PATH,
        };

        if (!cardPaymentMethods.length) {
            const noCardsTitle = otherPaymentMethods.length === 1
                ? this.SAVED_PAYMENT_METHOD_TITLE
                : this.PAYMENT_METHODS_LIST_TEXT;
            this.renderOtherMpMethodsBlock(otherPaymentMethods, noCardsTitle, blockHeader);
            return;
        }

        const savedCardsTitle = cardPaymentMethods.length === 1
            ? this.SAVED_CARD_TITLE
            : this.SAVED_CARDS_TITLE;

        // Block 2 renders first so Block 1 ends up on top via insertBefore(firstChild)
        this.renderOtherMpMethodsBlock(otherPaymentMethods);
        this.renderSavedCardsBlock(cardPaymentMethods, blockHeader, savedCardsTitle);
    }
